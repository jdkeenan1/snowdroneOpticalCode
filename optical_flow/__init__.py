...
class OpticalFlowCalculator:
    ...
    def processFrame(self, frame, distance=None, timestep=1):
        ...
        marginY = marginX
        marginZeroX = 5
        marginZeroY = marginZeroX
        
        if self.prev_gray != None:

            flow = cv2.calcOpticalFlowFarneback(self.prev_gray, gray, None,0.5, 5, 13, 10, 5, 1.1, flags=0) 
            arrayAll = []
            arrayX = []
            arrayMedian = []
            def arrayMedianFunction(inputValue):
		arrayX1 = []
		arrayX2 = []
		for row in arrayX:
                    if ((row[0] > marginZeroX) or (row[0] < -marginZeroX)):
                        arrayX1.append(row[0])
                    if ((row[1] > marginZeroX) or (row[1] < -marginZeroX)):
                        arrayX2.append(row[1])
                return [np.median(arrayX1),np.median(arrayX2)]
            for y in range(0, flow.shape[0], self.move_step):
                for x in range(0, flow.shape[1], self.move_step):
                    ...
                    xsum += fx
                    ysum += fy
                arrayAll.append(arrayX)
                arrayMedian.append(arrayMedianFunction(arrayX))
                arrayX = []
                    #cv2.line(frame2, (x,y), (int(x+fx),int(y+fy)), self.mv_color_bgr)
                    #cv2.circle(frame2, (x,y), 1, self.mv_color_bgr, -1)
            #now we find the values that are weird...
	    
            i = 0
            for y in range(0, flow.shape[0], self.move_step):
                j = 0
                for x in range(0, flow.shape[1], self.move_step):
                    if (((arrayAll[i][j][0] > (arrayMedian[i][0]+marginX)) or (arrayAll[i][j][0] < (arrayMedian[i][0]-marginX)) or (arrayAll[i][j][1] > (arrayMedian[i][1]+marginY)) or (arrayAll[i][j][1] < (arrayMedian[i][1]-marginY))) and ((arrayAll[i][j][0] > (marginZeroX)) or (arrayAll[i][j][0] < (-marginZeroX) ) or (arrayAll[i][j][1] > (marginZeroY)) or (arrayAll[i][j][1] < (-marginZeroY)))):
                        #print arrayAll[i][j],arrayMedian[i],arrayAll[i]
                        cv2.line(frame2, (x,y), (int(x+arrayAll[i][j][0]),int(y+arrayAll[i][j][1])), self.mv_color_bgr)
                        cv2.circle(frame2, (x,y), 1, self.mv_color_bgr, -1)
                    j += 1
                i += 1
