...
if __name__=="__main__":
    ...
    while True:
        ...
        xvel, yvel, result = flow.processFrame(frame)
	#frames.append(result)
        #if not result:
        #    break
        if count == 500:
	    break
        outputString = './images/'+str(count)+'.png'
	cv2.imwrite(outputString, result)

    #frames.reverse()
    elapsed_sec = time.time() - start_sec

    #writeGif("reverse_" + os.path.basename(options.filename), frames, duration=elapsed_sec/1000.0, dither=0)
